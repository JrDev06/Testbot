  ...
  async execute(api, event, args) {
    ...
    try {
      ...
      if (searchResponse.data && searchResponse.data.track) {
        ...
        const lyricsMessage = lyrics ? convertToGothic(`Lyrics for "${lyrics.title}" by ${lyrics.artist}:\n${lyrics.lyrics}`) : convertToGothic("Lyrics not found.");
        ...
        await api.sendMessage(convertToGothic("No results found. Please try again."), event.threadID, event.messageID);
      }
    } catch (error) {
      console.error(error);
      await api.sendMessage(convertToGothic("An error occurred while searching for the track."), event.threadID, event.messageID);
    }
  },
};